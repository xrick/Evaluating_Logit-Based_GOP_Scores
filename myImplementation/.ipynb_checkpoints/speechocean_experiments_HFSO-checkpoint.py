    ...
    with open(csv_output, "w", newline="") as f:
        ...
        for idx, example in enumerate(data):
            ...
            for word in example.get("words", []):
                ...
                # If there are mispronunciations, use them, otherwise use actual phones
                if mispronunciations:
                    # For simplicity, we'll use the first mispronunciation if available
                    mispronounced_phonemes.extend(mispronunciations[0] if mispronunciations else phones)
                else:
                    mispronounced_phonemes.extend(phones)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv_output", default="output/myimpl_speechocean_metrics_withHFSO.csv")
