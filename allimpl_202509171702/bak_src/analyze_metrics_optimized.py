# myimpl/analyze_metrics_optimized.py
import pandas as pd
import numpy as np
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, matthews_corrcoef

def analyze_gop_metrics(csv_filepath: str, max_thresholds: int = 1000):
    """
    ...
    Args:
        ...
        max_thresholds (int): 最大測試閾值數量，用於優化效能。
    """
    try:
    ...
    for metric, direction in metrics_to_evaluate.items():
        ...
        if len(df_metric) < original_count:
            print(f"指標 '{metric}': 移除了 {original_count - len(df_metric)} 個含有 NaN 或 inf 的資料點。")
        ...
        # 優化閾值搜尋：使用分位數採樣而非所有唯一值
        unique_scores = scores.unique()
        if len(unique_scores) > max_thresholds:
            print(f"指標 '{metric}': 有 {len(unique_scores)} 個唯一值，使用 {max_thresholds} 個分位數閾值進行優化。")
            # 使用分位數來選擇代表性閾值
            percentiles = np.linspace(0, 100, max_thresholds)
            thresholds = np.percentile(scores, percentiles)
            thresholds = np.unique(thresholds)  # 移除重複值
        else:
            thresholds = unique_scores

        print(f"測試 {len(thresholds)} 個閾值...")

        best_mcc = -1
        best_threshold = None

        for i, threshold in enumerate(thresholds):
            if i % 100 == 0:  # 每100個閾值顯示進度
                print(f"  進度: {i}/{len(thresholds)}")
        ...
        if best_threshold is not None:
            ...
            print(f"指標 '{metric}' 完成，最佳 MCC: {best_mcc:.4f}")
        else:
            print(f"指標 '{metric}': 無法找到有效的閾值。")
