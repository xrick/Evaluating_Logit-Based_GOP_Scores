# myimpl/speechocean_experiments_so5000.py
# myimpl/speechocean_experiments_HFSO copy.py
# myimpl/speechocean_experiments_HFSO.py
# myimpl/speechocean_experiments_loadhf.py
# myimpl/speechocean_experiments copy.py
# myimpl/speechocean_experiments.py
# ==============================================================================
...
def load_audio(audio_input: "object", samplerate: int) -> np.ndarray:
    ...
    - torchcodec AudioDecoder (new HF datasets style)
    ...
    if hasattr(audio_input, 'get_all_samples'):
        try:
            ...
            data = audio_samples.data.numpy()
            sr = audio_samples.sample_rate

            # Handle multi-channel audio (convert to mono)
            if data.ndim > 1:
                if data.shape[0] == 1:  # (1, samples)
                    data = data.squeeze(0)
            ...
            if sr != samplerate:
                try:
                    ...
                    print(f"Warning: librosa not available, using audio at {sr}Hz instead of {samplerate}Hz")

            return data.astype(np.float32)
        except Exception as e:
            print(f"Error processing torchcodec audio: {e}")
...
def run_experiment(
    ...
    with open(csv_output, "w", newline="") as f:
        ...
        for idx, example in enumerate(data):
            uttid = example.get("speaker", idx)

            # Extract phonemes from words structure
            actual_phonemes = []
            ipa_phonemes = []
            phone_accuracies = []
            ...
            for word in example.get("words", []):
                ...
                actual_phonemes.extend(phones)
                # Convert to IPA format for model compatibility
                ipa_phones = [convert_arpabet_phone(phone) for phone in phones]
                ipa_phonemes.extend(ipa_phones)
                phone_accuracies.extend(accuracies)

            # Create mispronounced labels based on accuracy threshold
            # Threshold of 7: scores <= 7 are considered mispronounced
            MISPRONUNCIATION_THRESHOLD = 7.0
            mispronounced_flags = []
            for acc in phone_accuracies:
                if acc is not None:
                    mispronounced_flags.append(acc <= MISPRONUNCIATION_THRESHOLD)
                else:
                    mispronounced_flags.append(False)  # Default to correct if no accuracy score

            audio = load_audio(example["audio"], samplerate)
            human_accuracies = example.get("phoneme_accuracies", [None] * len(actual_phonemes))

            spans = ctc_align_phonemes(audio, ipa_phonemes, feat_extractor, tokenizer, model, samplerate, device)
            ...
            for i, actual in enumerate(actual_phonemes):
                if i >= len(spans):
                    continue
                s = spans[i]
                ...
                # Use IPA phoneme for vocabulary lookup
                ipa_phoneme = ipa_phonemes[i] if i < len(ipa_phonemes) else actual
                target_id = vocab.get(ipa_phoneme)
                import numpy as _np
                metrics = {
                ...
                is_mispronounced = mispronounced_flags[i] if i < len(mispronounced_flags) else False
                writer.writerow([
                    uttid,
                    actual,
                    actual,  # Keep actual phoneme for reference
                    s["start_time"],
                    s["end_time"],
                    s["conf"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", required=False, default="../dataset/speechocean762-combined-hf-format", help="Path to HF dataset on disk (load_from_disk)")
    parser.add_argument("--csv_output", default="output/myimpl_speechocean_metrics_so5000_v2.csv")
