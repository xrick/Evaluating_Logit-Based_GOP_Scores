...
# official/speechocean_quantification.py
import logging
import torch
import numpy as np
import pandas as pd
...
def create_csv_data(
    ...
    with open(csv_output, mode="w", newline="") as csvfile:
        ...
        for example in tqdm(data, desc="Processing examples"):
            ...
            # Handle audio data from HuggingFace datasets with torchcodec AudioDecoder
                ...
                if hasattr(audio_data, 'get_all_samples'):
                    ...
                    if sr != samplerate:
                        try:
                            ...
                            logger.warning(f"librosa not available, using audio at {sr}Hz instead of {samplerate}Hz")
                            # Update samplerate for this sample
                    ...
                    # Very old format
                    audio = audio_data["array"]
                    sr = samplerate  # Assume correct sample rate

                else:
                    logger.error(f"Unsupported audio format: {type(audio_data)}")
                    continue

            except Exception as e:
                logger.error(f"Error processing audio for speaker {uttid}: {e}")
                continue
...
if __name__ == "__main__":
    ...
    logger.info("Loading dataset from %s", "HuggingFace")
    # so_data = load_from_disk(DATA_PATH)
    so_data = load_dataset("mispeech/speechocean762", split="train")
